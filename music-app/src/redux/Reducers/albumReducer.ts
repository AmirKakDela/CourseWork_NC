...
const albumReducer = (state = initialState, action: AlbumAction): State => {
    switch (action.type) {
        ...
        //     return {
        //         ...state,
        //         albums: state.albums.map( album => album._id === action.payload as string ? { ...album, ...action.payload.changes } : album)
        //     }
        case AlbumActionsType.DELETE_ALBUM_REQUEST:
            return {
                ...state,
                message: action.payload as string || initialState.message,
                albums: state.albums.filter(album => album._id !== action.payload as string)
            }
